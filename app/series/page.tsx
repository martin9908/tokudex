...
const franchiseMeta: Record<string, { years: string; genres: string }> = {
    "Kamen Rider": { years: "2000-2001", genres: "Action, Sci-Fi, Superhero" },
    "Super Sentai": { years: "2023-2024", genres: "Action, Adventure, Team" },
    Ultraman: { years: "2023", genres: "Action, Kaiju, Sci-Fi" },
};
...
function getReleaseYear(series: Series): number {
    ...
    const franchiseYears = series.franchise
        ? franchiseMeta[series.franchise]?.years
        : undefined;
    const fallbackYear = franchiseYears?.match(/(19|20)\d{2}/)?.[0];
...
export default function SeriesListPage() {
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-6 pb-24 md:pb-10 w-full space-y-5">
            <div className="rounded-2xl border border-cyan-950/70 bg-[#071423]/75 backdrop-blur-xl px-4 py-3 flex flex-col sm:flex-row gap-3 sm:items-center">
            ...
            {featuredSeries ? (
                <>
                    <section className="relative overflow-hidden rounded-3xl border border-cyan-900/60 bg-[#050d17] min-h-[560px]">
                        ...
                        <div className="relative z-10 p-6 md:p-9 h-full flex flex-col">
                            <div className="max-w-2xl">
                                <p className="text-xs tracking-[0.35em] text-red-400/90 font-semibold uppercase mb-5">
                                ...
                                <div className="rounded-2xl border border-cyan-900/50 bg-[#061321]/70 px-4 py-3 mb-6 flex items-center justify-between max-w-md">
                                    <div>
                                        <p className="text-sm text-cyan-100 font-semibold">
                                            Tracker Actions
                                        </p>
                                        <p className="text-xs text-cyan-200/60">Log progress or open details</p>
                                    </div>
                                    <Link
                                ...
                                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-cyan-200/80">
                                    <span>{featuredSeries.original_run || (featuredSeries.franchise ? franchiseMeta[featuredSeries.franchise]?.years || "N/A" : "N/A")}</span>
                                    <span className="opacity-40">|</span>
                                    <span>{featuredSeries.total_episodes || "?"} Episodes</span>
                                    <span className="opacity-40">|</span>
                                    <span>{featuredSeries.franchise ? franchiseMeta[featuredSeries.franchise]?.genres || "N/A" : "N/A"}</span>
                                </div>
                            </div>

                            <div className="mt-7 rounded-2xl max-w-2xl border border-cyan-900/40 bg-[#071627]/65 p-5">
                                <p className="text-[11px] tracking-[0.28em] uppercase text-cyan-200/60 mb-4">
                                    Tracking Overview
                                </p>
                                ...
                                <div className="h-2 w-full rounded-full bg-cyan-950/80 overflow-hidden">
                                    <div
                    ...
                    <section className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-4">
                        <article className="rounded-2xl border border-cyan-900/45 bg-[#071423]/70 p-4 md:p-5">
                        ...
                        <aside className="rounded-2xl border border-cyan-900/45 bg-[#071423]/70 p-4 md:p-5">
                            <p className="text-cyan-100 text-lg font-semibold mb-4">
                                About The Series
                            </p>
                            <dl className="space-y-3 text-sm">
                                <div>
                                    <dt className="text-cyan-200/60">Franchise</dt>
                                    <dd className="text-cyan-100">{featuredSeries.franchise || "N/A"}</dd>
                                </div>
                                <div>
                                    <dt className="text-cyan-200/60">Total Episodes</dt>
                                    <dd className="text-cyan-100">{featuredSeries.total_episodes || "N/A"}</dd>
                                </div>
                                <div>
                                    <dt className="text-cyan-200/60">Original Run</dt>
                                    <dd className="text-cyan-100">
                                        {featuredSeries.original_run || (featuredSeries.franchise ? franchiseMeta[featuredSeries.franchise]?.years || "N/A" : "N/A")}
                                    </dd>
                                </div>
                                <div>
                                    <dt className="text-cyan-200/60">Updated</dt>
                                    <dd className="text-cyan-100">{latestUpdatedLabel}</dd>
                                </div>
                            </dl>
                        </aside>
                    </section>
                    ...
                    <section className="pt-3">
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <p className="text-xs text-cyan-200/60">Library</p>
                        </div>
